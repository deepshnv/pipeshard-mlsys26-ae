...
void llama_model_loader::get_mapping_range_all(size_t * first, size_t * last, void ** addr, int idx, ggml_context * ctx) const {
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (use_mmap) {
            ...
            if (ggml_backend_buffer_is_host(cur->buffer)) {
                ...
                tensor_data_map[ggml_get_name(cur)] = cur->data;
                if (check_tensors) {
...
void * llama_model_loader::get_tensor_data(const std::string & name) const {
    auto it = tensor_data_map.find(name);
