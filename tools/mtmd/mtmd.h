...
struct mtmd_context_params {
    ...
    // VTO params
    int32_t clip_img_size;
    bool vto_offload_cpu;
    bool vto_tiled_attention;
    int32_t clip_tiled_mb;
};
...
// JIT LLM init: register a callback invoked after image encode (for VTO deferred context init)
...
// Whether VTO pre-encode/JIT flow is enabled
MTMD_API bool mtmd_preencode_enabled(mtmd_context * ctx);
// Pre-encode the first IMAGE chunk to trigger CLIP VRAM release before llama_decode
MTMD_API int32_t mtmd_preencode_first_image(mtmd_context * ctx, const mtmd_input_chunks * chunks);
...
// Retrieve encode timing (ms) for media chunk
