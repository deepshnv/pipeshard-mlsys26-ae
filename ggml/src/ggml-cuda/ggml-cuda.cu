...
static bool ggml_cuda_use_pipeline_sharding() {
    static int cached = -1;
    if (cached == -1) {
        const char* env_value = getenv("GGML_CUDA_PIPELINE_SHARDING");
        cached = 0;
        ...
        if (cached) {
            GGML_LOG_INFO("CUDA pipeline sharding enabled\n");
        }
    }
    return cached == 1;
}
...
void ggml_cuda_set_device(int device) {
    ...
    if (ggml_cuda_use_pipeline_sharding()) {
        // Pipeline sharding: always use device 0
...
static ggml_cuda_device_info ggml_cuda_init() {
    ...
    if (ggml_cuda_use_pipeline_sharding()) {
         // Pipeline sharding: create virtual devices
        GGML_LOG_INFO("Creating %d virtual CUDA devices for pipeline sharding\n", GGML_CUDA_MAX_DEVICES);
        total_vram = total_vram/2*GGML_CUDA_MAX_DEVICES;
        info.devices[0].total_vram = total_vram;
        info.device_count = GGML_CUDA_MAX_DEVICES;
        for (int i = 1; i < GGML_CUDA_MAX_DEVICES; i++)
        {
            info.devices[i].cc = info.devices[0].cc;
            info.devices[i].nsm = info.devices[0].nsm;
            info.devices[i].smpb = info.devices[0].smpb;
            info.devices[i].smpbo = info.devices[0].smpbo;
            info.devices[i].vmm = info.devices[0].vmm;
            info.devices[i].vmm_granularity = info.devices[0].vmm_granularity;
            info.devices[i].total_vram = info.devices[0].total_vram;
        }
    }
...
struct ggml_cuda_pool_vmm : public ggml_cuda_pool {
    const size_t CUDA_POOL_VMM_MAX_SIZE;  // 32 GB
    ...
    explicit ggml_cuda_pool_vmm(int device) :
        CUDA_POOL_VMM_MAX_SIZE(1ull << 35),
        device(ggml_cuda_use_pipeline_sharding() ? 0 : device),
...
static void ggml_backend_cuda_get_tensor_async(ggml_backend_t backend, const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    if (!ggml_cuda_use_pipeline_sharding()){
		...
		ggml_backend_buffer_type_t buft = tensor->view_src ? tensor->view_src->buft : tensor->buft;
		GGML_ASSERT(buft == ggml_backend_cuda_buffer_type(cuda_ctx->device) && "unsupported buffer type"); 																   	
	}
...
#else
            if (ggml_cuda_use_pipeline_sharding()) {
                // Pipeline sharding: use regular device-to-device copy
                CUDA_CHECK(cudaMemcpyAsync(dst->data, src->data, ggml_nbytes(dst), cudaMemcpyDeviceToDevice, cuda_ctx_src->stream()));
            } else {
...
void * ggml_backend_cuda_get_stream_ptr(ggml_backend_t backend) {
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
	...
	if(ggml_cuda_use_pipeline_sharding()){
		for (int i = 0; i < GGML_MAX_SRC; i++) {
			if (op->src[i] && op->src[i]->buft && ggml_backend_buft_is_cuda(op->src[i]->buft)) {
				ggml_backend_cuda_buffer_type_context * buft_ctx = (ggml_backend_cuda_buffer_type_context *)op->src[i]->buft->context;
...

    {
        ...
        if (!initialized) {
            ...
            for (int i = 0; i < ggml_cuda_info().device_count; i++) {
                ...
                cudaDeviceProp prop;
                CUDA_CHECK(cudaGetDeviceProperties(&prop, ggml_cuda_use_pipeline_sharding() ? 0 : i));
