...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        { "-mva", "--max-vram-alloc" }, "N", "provide max vram allocation",
        [](common_params & params, int value) {
            params.max_vram_alloc = value;
            if (!llama_supports_gpu_offload()) {
                fprintf(stderr,
                        "warning: not compiled with GPU offload support, --extra-vram-alloc option will be ignored\n");
                fprintf(stderr, "warning: see main README.md for information on enabling GPU BLAS support\n");
            }
        }));
    add_opt(common_arg(
        { "-psa", "--pinned-sysmem-alloc" }, "N", "provide max pinned allocation",
        [](common_params & params, int value) {
            params.pinned_sysmem_alloc = value;
            if (!llama_supports_gpu_offload()) {
                fprintf(stderr,
                        "warning: not compiled with GPU offload support, --extra-vram-alloc option will be ignored\n");
                fprintf(stderr, "warning: see main README.md for information on enabling GPU BLAS support\n");
    ...
    add_opt(common_arg(
        ...
        "pipe-shard",
        [](common_params& params) {
            const char * ggml_cuda_pipeline_sharding = getenv("GGML_CUDA_PIPELINE_SHARDING");
            if (ggml_cuda_pipeline_sharding != NULL && strcmp(ggml_cuda_pipeline_sharding, "1") == 0) {
                params.pipe_shard   = true;
                params.n_gpu_layers = 1000000;
                params.flash_attn   = true;
            }
        }
    ));
    add_opt(common_arg({ "-cis", "-clip-img-size" }, "N",
        "override max image dimension for models like qwen2.5-vl to run at native res (default: -1 = model default)",
        [](common_params & params, int value) {
            params.clip_img_size = value;
        }
    ));
    add_opt(common_arg({ "-vto-off", "-vto-offload-cpu" },
        "offload clip weights to CPU and stream at runtime (default: false)",
        [](common_params & params) {
            params.vto_offload_cpu = true;
        }
    ));
    add_opt(common_arg({ "-vto-tile", "-vto-tiled-attention" },
        "reduce peak VRAM of O(N^2) KQ,V tensors by processing Q in smaller tiles (default: false)",
        [](common_params & params) {
            params.vto_tiled_attention = true;
        }
    ));
    add_opt(common_arg({ "-clip-tiled-mb" }, "N",
        string_format("budget in MiB for CLIP tiled Q attention (default: %d)", params.clip_tiled_mb),
        [](common_params & params, int value) {
            if (value < 128) {
                fprintf(stderr, "warning: -clip-tiled-mb too low (%d), clamping to 128 MiB\n", value);
                value = 128;
            }
            params.clip_tiled_mb = value;
        }
    ));
    add_opt(common_arg(
        { "-fes", "--force-exec-strategy" }, "N",
        "force execution strategy (0=FULL_GPU, 1=FULL_GPU_NO_OUTPUT, 2=STATIC_ATTN_PRIO, 3=SHARD_ATTN_PRIO, 4=FULL_SHARD), -1=auto",
        [](common_params & params, int value) {
            params.force_exec_strategy = value;
        }
    ));
    add_opt(common_arg(
        {"--cpu-profile"},
        "PATH",
        "path to CPU benchmark profile file for pipeline executor (default: concurrent_results.txt)",
        [](common_params & params, const std::string & value) {
            params.cpu_profile_path = value;
        }
    ));
    add_opt(common_arg(
        {"--gpu-profile"},
        "PATH",
        "path to GPU benchmark profile file for pipeline executor (default: gpu_results.txt)",
        [](common_params & params, const std::string & value) {
            params.gpu_profile_path = value;
        }
    ));
	add_opt(common_arg(
        {"--spm-infill"},
    ...
    add_opt(common_arg(
        {"-mvad", "--max-vram-alloc-draft"}, "N",
        "VRAM budget in MB for the draft model with pipeshard (0 = use n_gpu_layers instead)",
        [](common_params & params, int value) {
            params.speculative.max_vram_alloc = value;
            if (!llama_supports_gpu_offload()) {
                fprintf(stderr, "warning: no usable GPU found, --max-vram-alloc-draft option will be ignored\n");
                fprintf(stderr, "warning: one possible reason is that llama.cpp was compiled without GPU support\n");
                fprintf(stderr, "warning: consult docs/build.md for compilation instructions\n");
            }
        }
