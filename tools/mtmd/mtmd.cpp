...
mtmd_context_params mtmd_context_params_default() {
    ...
    params.clip_img_size       = -1;
    params.vto_offload_cpu     = false;
    params.vto_tiled_attention = false;
    params.clip_tiled_mb       = 2048;
    return params;
}
...
struct mtmd_context {
    ...
    // JIT LLM init integration (when vto_offload_cpu || vto_tiled_attention)
    mtmd_llm_init_cb llm_init_cb        = nullptr;
    void *           llm_init_user_data = nullptr;
    llama_context *  llm_lctx           = nullptr;
        ...
        n_embd_text  (llama_model_n_embd(text_model))
    {
        ...
        ctx_clip_params.clip_img_size       = ctx_params.clip_img_size;
        ctx_clip_params.vto_offload_cpu     = ctx_params.vto_offload_cpu;
        ctx_clip_params.vto_tiled_attention = ctx_params.vto_tiled_attention;
        ctx_clip_params.clip_tiled_mb       = ctx_params.clip_tiled_mb;
...
extern "C" MTMD_API bool mtmd_preencode_enabled(mtmd_context * ctx) {
    return ctx->vto_offload_cpu || ctx->vto_tiled_attention;
}

extern "C" MTMD_API int32_t mtmd_preencode_first_image(mtmd_context * ctx,
...
extern "C" MTMD_API int64_t mtmd_get_image_encode_timing(mtmd_context * ctx,
