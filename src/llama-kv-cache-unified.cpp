...
llama_kv_cache_unified::llama_kv_cache_unified(
    ...
    if (model.params.pipe_shard) {
        pipe_shard_kv = std::make_unique<llama_kv_cache_pipe_shard>();
        pipe_shard_kv->init(this, model, hparams, type_k, type_v, kv_size, n_stream, n_layer_cache, v_trans);
        return;
    }
...
void llama_kv_cache_unified::clear(bool data) {
    ...
    if (data) {
        if (pipe_shard_kv) {
            return pipe_shard_kv->clear_buffers();
        }
        for (auto & buf : bufs) {
...
bool llama_kv_cache_unified::update(llama_context * lctx, bool do_shift, const defrag_info & dinfo, const stream_copy_info & sc_info) {
    ...
    if (do_shift) {
        ...
        if (hparams.rope_type != LLAMA_ROPE_TYPE_NONE) {
            ...
            // For pipe-shard: use CPU-only path that bypasses scheduler entirely
            if (model.params.pipe_shard) {
                // Activate CPU KV for all layers
                for (uint32_t il = 0; il < hparams.n_layer; ++il) {
                    activate_cpu_kv(il);
                }

                // Build the shift graph (uses CPU K tensors)
                auto * gf = build_graph_shift(res, lctx);

                // Find CPU backend from scheduler
                ...
                GGML_ASSERT(cpu_backend != nullptr && "CPU backend not found in scheduler");

                // Create a CPU-only gallocr for scratch tensors
                ggml_gallocr_t cpu_galloc = ggml_gallocr_new(ggml_backend_get_default_buffer_type(cpu_backend));

                // Reserve and allocate the graph on CPU only
                if (!ggml_gallocr_reserve(cpu_galloc, gf)) {
                    LLAMA_LOG_ERROR("%s: failed to reserve CPU graph for K-shift\n", __func__);
                    ggml_gallocr_free(cpu_galloc);
                    return updated;
                }

                if (!ggml_gallocr_alloc_graph(cpu_galloc, gf)) {
                    LLAMA_LOG_ERROR("%s: failed to allocate CPU graph for K-shift\n", __func__);
                    ggml_gallocr_free(cpu_galloc);
                    return updated;
                }
                ...
                if (ggml_backend_graph_compute(cpu_backend, gf) != GGML_STATUS_SUCCESS) {
                    LLAMA_LOG_ERROR("%s: failed to compute K-shift on CPU\n", __func__);
                    ggml_gallocr_free(cpu_galloc);
                    return updated;
                }

                // Free the temporary allocator
                ggml_gallocr_free(cpu_galloc);

                updated = true;
            } else {
                // Normal path: use scheduler
                ggml_backend_sched_reset(sched);

                auto * gf = build_graph_shift(res, lctx);
                if (!ggml_backend_sched_alloc_graph(sched, gf, true)) {
                ...
                if (lctx->graph_compute(gf, false, true) != GGML_STATUS_SUCCESS) {
...
size_t llama_kv_cache_unified::total_size() const {
    if (pipe_shard_kv) {
        return pipe_shard_kv->get_total_size();
    }
...
void llama_kv_cache_unified::activate_cpu_kv(int layer_num) {
    if (pipe_shard_kv) {
        pipe_shard_kv->activate_cpu(this, layer_num);
    }
}

void llama_kv_cache_unified::activate_gpu_kv(int layer_num) {
    if (pipe_shard_kv) {
        pipe_shard_kv->activate_gpu(this, layer_num);
    }
}

ggml_tensor * llama_kv_cache_unified::get_k_l_tensor(int layer_num) {
    return layers.at(map_layer_ids.at(layer_num)).k;
}

ggml_tensor * llama_kv_cache_unified::get_v_l_tensor(int layer_num) {
    return layers.at(map_layer_ids.at(layer_num)).v;
}

ggml_tensor * llama_kv_cache_unified::get_k_l_cpu_tensor(int layer_num) {
    return pipe_shard_kv ? pipe_shard_kv->k_cpu(layer_num) : nullptr;
}

ggml_tensor * llama_kv_cache_unified::get_v_l_cpu_tensor(int layer_num) {
    return pipe_shard_kv ? pipe_shard_kv->v_cpu(layer_num) : nullptr;
}

ggml_tensor * llama_kv_cache_unified::get_k_l_gpu_tensor(int layer_num) {
    return pipe_shard_kv ? pipe_shard_kv->k_gpu(layer_num) : nullptr;
}

ggml_tensor * llama_kv_cache_unified::get_v_l_gpu_tensor(int layer_num) {
    return pipe_shard_kv ? pipe_shard_kv->v_gpu(layer_num) : nullptr;
}

size_t llama_kv_cache_unified::max_layer_size_k_bytes() const {
    return pipe_shard_kv ? pipe_shard_kv->size_k_bytes_max() : 0;
}

size_t llama_kv_cache_unified::max_layer_size_v_bytes() const {
    return pipe_shard_kv ? pipe_shard_kv->size_v_bytes_max() : 0;
}
