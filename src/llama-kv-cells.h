...
public:
    ...
    // Get all used cell indices
    const std::set<uint32_t> & get_used_cells() const { return used; }

    // Get cells belonging to a specific sequence
    std::vector<uint32_t> get_cells_for_seq(llama_seq_id seq_id) const {
        std::vector<uint32_t> result;
        result.reserve(used.size());  // worst case
        for (uint32_t i : used) {
            if (seq[i].test(seq_id)) {
                result.push_back(i);
            }
    ...
    // Check if a cell belongs to a specific sequence
    bool cell_has_seq(uint32_t cell_idx, llama_seq_id seq_id) const {
        if (cell_idx >= seq.size()) {
            return false;
        }
        return seq[cell_idx].test(seq_id);
    }
