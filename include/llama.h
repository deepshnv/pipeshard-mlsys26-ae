...
#include "ggml-opt.h"
#include "../ggml/src/ggml-impl.h" // pipeshard: need to access graph in llama-context												  
...
extern "C" {
    ...
    struct llama_context_params {
        ...
        size_t max_vram_alloc;     // The maximum VRAM to allocate for the pipeline.
        size_t pinned_sysmem_alloc;
        bool pipe_shard;           // Enable pipeline sharding.
        bool has_lora;             // Whether LoRA is enabled.
        int force_exec_strategy;   // -1 = auto, 0-5 = force specific exec_strategy
        bool is_vlm;
        uint32_t n_draft;          // max speculative draft tokens (0 = not speculative, used by pipeshard for tier planning)
        const char * cpu_profile_path;
        const char * gpu_profile_path;
    };
