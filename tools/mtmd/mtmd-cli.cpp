...
struct mtmd_cli_context {
    ...
    int n_threads    = 1;
    llama_pos n_past = 0;
    common_params saved_params;  // kept for JIT LLM init (VTO flow)

    mtmd_cli_context(common_params & params)
    {
        if (params.vto_offload_cpu || params.vto_tiled_attention)
        {
            // VTO path: defer LLM context creation; load model only for chat template
            saved_params = params;
            ...
            // Register JIT callback invoked after CLIP encode frees VRAM
    ...
    {
        // Reuse the already loaded model to avoid re-reading the GGUF
        ...
        llama_init.lora    = std::move(init2.lora);
        model      = llama_init.model.get();
        lctx       = llama_init.context.get();
        vocab      = llama_model_get_vocab(model);
        smpl       = common_sampler_init(model, params.sampling);
    ...
    void init_vision_context(common_params & params) {
        ...
        mparams.clip_img_size       = params.clip_img_size;
        mparams.vto_offload_cpu     = params.vto_offload_cpu;
        mparams.vto_tiled_attention = params.vto_tiled_attention;
        mparams.clip_tiled_mb       = params.clip_tiled_mb;
