...
struct server_context {
    ...
    bool load_model(const common_params & params) {
        ...
        if (!params_base.speculative.model.path.empty() || !params_base.speculative.model.hf_repo.empty()) {
            ...
            // pipeshard: give draft model its own VRAM budget (single-slot only)
            if (params_base.pipe_shard && params_base.n_parallel == 1) {
                params_dft.max_vram_alloc = params_base.speculative.max_vram_alloc;
                params_dft.n_gpu_layers   = 1000000;  // pipeshard manages layer placement
                params_dft.flash_attn     = true;
            } else if (params_base.pipe_shard) {
                // n_parallel > 1 or no draft VRAM budget: each slot creates its own ctx_dft,
                // so pipeshard would allocate n_parallel separate VRAM buffers. Fall back to CPU.
                if (params_base.n_parallel > 1 && params_base.speculative.max_vram_alloc > 0) {
                    SRV_WRN("pipeshard draft with n_parallel=%d would allocate %d VRAM buffers, falling back to CPU draft\n",
                            params_base.n_parallel, params_base.n_parallel);
                }
                params_dft.max_vram_alloc = 0;
                params_dft.n_gpu_layers   = 1000000;  // pipeshard manages layer placement
                params_dft.flash_attn     = true;
            }
