...
struct llama_model {
    ...
    mutable std::vector <bool> use_separate_cuda_host_buf;
    mutable std::vector<ggml_backend_buffer_ptr> host_bufs;
    std::map<int32_t, std::vector<struct ggml_tensor *>> layer_to_tensor_map;
	...
	ggml_backend_dev_t dev_input() const;									 
    ...
    void* get_mapping_addr(int i) const;
    ggml_backend_buffer_t get_cpu_buf() const;							  

    std::map<ggml_tensor*, void*> hybrid_tensor_data;
    std::map<ggml_tensor*, bool> tensor_pin_decisions;
private:
    struct impl;
