...
struct clip_ctx {
    ...
    std::vector<ggml_backend_buffer_ptr> additional_buffers;  // for vto tensor overrides

    int max_nodes = 8192;
    ggml_backend_sched_ptr sched;

    int32_t clip_img_size        = -1;
    bool    vto_offload_cpu      = false;
    bool    vto_tiled_attention  = false;
    int32_t clip_tiled_mb        = 2048;
    ...
    clip_ctx(clip_context_params & ctx_params) {
        ...
        clip_img_size       = ctx_params.clip_img_size;
        vto_offload_cpu     = ctx_params.vto_offload_cpu;
        vto_tiled_attention = ctx_params.vto_tiled_attention;
        clip_tiled_mb       = ctx_params.clip_tiled_mb > 0 ? ctx_params.clip_tiled_mb : 2048;
    }
    ...
    ggml_cgraph * build_qwen2vl() {
        ...
        for (int il = 0; il < n_layer; il++) {
            ...
            // self-attention
            {
                ...
                // VTO tiled attention: tile queries to reduce peak VRAM of O(N^2) attention tensors
                if (ctx->vto_tiled_attention && !warmup_pass)
                {
                    const int64_t num_positions = Qcur->ne[2];
                    const int64_t n_head_local  = Qcur->ne[1];

                    size_t       budget_mib    = (size_t) ctx->clip_tiled_mb;
                    const size_t budget_bytes  = budget_mib * (size_t) 1024 * 1024;
                    const size_t element_bytes = 4;  // F32 activations

                    size_t       q_tile        = (size_t) (budget_bytes / (element_bytes * (size_t) n_head_local * (size_t) num_positions));
                    if (use_fa)
                    {
                        size_t tot_run_overhead = static_cast<size_t>((fa_attn_tensor_overheads_4k + non_attn_tensor_overheads_4k) * img_ratio_wrt_4k);
                        if (budget_bytes > tot_run_overhead)
                        {
                            q_tile = num_positions;
                        }
                    }

                    if ((int64_t) q_tile < 64)        q_tile = 64;
                    if ((int64_t) q_tile > num_positions) q_tile = (size_t) num_positions;

                    // Prevent mask operations from exceeding 2GB ggml_cpy limit
                    const int64_t n_kv = num_positions;
                    if ((attn_mask != nullptr || !full_attn) && !mtmd_clip_use_full_flash_attn())
                    {
                        size_t mask_element_bytes = 4;
                        if (attn_mask != nullptr) mask_element_bytes = ggml_type_size(attn_mask->type);
                        const int64_t max_qlen_rounded = (INT_MAX / (n_kv * (int64_t)mask_element_bytes)) & ~63LL;
                        if (q_tile > (size_t)max_qlen_rounded) {
                            LOG_INF("%s: Limiting q_tile from %zu to %lld to keep mask under 2GB\n", __func__, q_tile, (long long)max_qlen_rounded);
                            q_tile = (size_t) max_qlen_rounded;
                        }
                    }

                    // round up to multiple of 64 and re-clamp
                    q_tile = ((q_tile + 63) / 64) * 64;
                    if ((int64_t) q_tile > num_positions) q_tile = (size_t) num_positions;

                    // Re-apply 2GB limit after rounding
                    if ((attn_mask != nullptr || !full_attn) && !mtmd_clip_use_full_flash_attn())
                    {
                        size_t mask_element_bytes = 4;
                        if (attn_mask != nullptr) mask_element_bytes = ggml_type_size(attn_mask->type);
                        ...
                        if (q_tile > (size_t)max_qlen_rounded) {
                            q_tile = (size_t)max_qlen_rounded;
                            if (q_tile < 64) {
                                LOG_ERR("%s: Cannot find safe q_tile - image too large for GGML 2GB limit. Try smaller --clip-image-size.\n", __func__);
                                GGML_ASSERT(false && "Image size exceeds GGML 2GB copy limit");
                            }
                        }
                    }

                    // Prepare K/V for FA once
                    ggml_tensor * k_fa_pre = nullptr;
                    ggml_tensor * v_fa_pre = nullptr;
                    if (use_fa || mtmd_clip_use_full_flash_attn())
                    {
                        k_fa_pre = ggml_permute(ctx0, Kcur, 0, 2, 1, 3);
                        v_fa_pre = ggml_permute(ctx0, Vcur, 0, 2, 1, 3);
                        if (k_fa_pre->type == GGML_TYPE_F32) k_fa_pre = ggml_cast(ctx0, k_fa_pre, GGML_TYPE_F16);
                        if (v_fa_pre->type == GGML_TYPE_F32) v_fa_pre = ggml_cast(ctx0, v_fa_pre, GGML_TYPE_F16);
                    }

                    if (q_tile >= num_positions || mtmd_clip_use_full_flash_attn())
                    {
                        if (use_fa) {
                            cur = build_attn_flash_kv_prepared(layer.o_w, layer.o_b, Qcur, k_fa_pre, v_fa_pre, attn_mask, kq_scale, il);
                        } else {
                    ...
                    {
                        ggml_tensor * out_cat = nullptr;
                        if (use_fa)
                        {
                            std::vector<ggml_tensor *> tile_2d_vec;
                            ggml_tensor * q_fa_full = nullptr;
                            if (((int64_t) q_tile) * 2 == num_positions) {
                                q_fa_full = ggml_permute(ctx0, Qcur, 0, 2, 1, 3);
                                q_fa_full = ggml_cont(ctx0, q_fa_full);
                            }
                            for (int64_t q0 = 0; q0 < num_positions; q0 += (int64_t) q_tile)
                            {
                                const int64_t qlen = std::min<int64_t>((int64_t) q_tile, num_positions - q0);
                                ggml_tensor * mask_sub = nullptr;
                                if (attn_mask) {
                                    const size_t m_off = (size_t) q0 * attn_mask->nb[1];
                                    mask_sub = ggml_view_2d(ctx0, attn_mask, attn_mask->ne[0], qlen, attn_mask->nb[1], attn_mask->view_offs + m_off);
                                    const int64_t need_pad = GGML_PAD(qlen, GGML_KQ_MASK_PAD);
                                    if (qlen < need_pad) {
                                        mask_sub = ggml_pad(ctx0, mask_sub, 0, (int)(need_pad - qlen), 0, 0);
                                    }
                                }
                                ggml_tensor * tile_2d = nullptr;
                                if (((int64_t) q_tile) * 2 == num_positions) {
                                    const size_t q_off_bytes_fa = (size_t) q0 * q_fa_full->nb[1];
                                    ggml_tensor * q_fa_tile = ggml_view_4d(ctx0, q_fa_full, d_head, qlen, n_head, batch_size,
                                        q_fa_full->nb[1], q_fa_full->nb[2], 0, q_fa_full->view_offs + q_off_bytes_fa);
                                    tile_2d = build_attn_flash_qkv_prepared(nullptr, nullptr, q_fa_tile, k_fa_pre, v_fa_pre, mask_sub, kq_scale, il);
                                } else {
                                    const size_t q_off_bytes_fa = (size_t) q0 * Qcur->nb[2];
                                    ggml_tensor * q_fa_tile = ggml_view_4d(ctx0, Qcur, d_head, qlen, n_head, batch_size,
                                        Qcur->nb[2], Qcur->nb[1], 0, Qcur->view_offs + q_off_bytes_fa);
                                    tile_2d = build_attn_flash_qkv_prepared(nullptr, nullptr, q_fa_tile, k_fa_pre, v_fa_pre, mask_sub, kq_scale, il);
                                }
                                tile_2d_vec.push_back(tile_2d);
                            }
                            for (size_t i = 0; i < tile_2d_vec.size(); i++) {
                                out_cat = (out_cat == nullptr) ? tile_2d_vec[i] : ggml_concat(ctx0, out_cat, tile_2d_vec[i], /*dim=*/1);
                            }
                        }
                        else
                        {
                            // Non-FA tiled path with per-head tiling
                            for (int64_t q0 = 0; q0 < num_positions; q0 += (int64_t) q_tile) {
                                const int64_t qlen = std::min<int64_t>((int64_t) q_tile, num_positions - q0);
                                ggml_tensor * mask_sub = nullptr;
                                if (attn_mask) {
                                ...
                                size_t q_off_bytes = (size_t) q0 * q->nb[1];
                                ggml_tensor * Q_chunk = ggml_view_3d(ctx0, q, q->ne[0], qlen, q->ne[2], q->nb[1], q->nb[2], q->view_offs + q_off_bytes);
                                Q_chunk = ggml_cont(ctx0, Q_chunk);
                                ggml_tensor * Q_chunk_4d = ggml_reshape_4d(ctx0, Q_chunk, d_head, qlen, n_head, batch_size);

                                ggml_tensor * k = ggml_permute(ctx0, Kcur, 0, 2, 1, 3);
                                ggml_tensor * v_mat = ggml_permute(ctx0, Vcur, 2, 1, 0, 3);
                                v_mat = ggml_permute(ctx0, v_mat, 0, 2, 1, 3);
                                v_mat = ggml_cont(ctx0, v_mat);
                                const int64_t n_kv_local = k->ne[1];
                                int64_t h_step = clip_calc_heads_per_step(budget_bytes, n_kv_local, qlen, n_head, element_bytes, img_ratio_wrt_4k);
                                if (h_step < 1) h_step = 1;

                                ggml_tensor * heads_cat = nullptr;
                                for (int64_t h0 = 0; h0 < n_head; h0 += h_step) {
                                    const int64_t hlen = std::min<int64_t>(h_step, n_head - h0);
                                    ggml_tensor * k_ht = ggml_view_4d(ctx0, k, d_head, k->ne[1], hlen, k->ne[3], k->nb[1], k->nb[2], k->nb[3], k->view_offs + (size_t) h0 * k->nb[2]);
                                    k_ht = ggml_cont(ctx0, k_ht);
                                    ggml_tensor * q_ht = ggml_view_4d(ctx0, Q_chunk_4d, d_head, qlen, hlen, Q_chunk_4d->ne[3], Q_chunk_4d->nb[1], Q_chunk_4d->nb[2], Q_chunk_4d->nb[3], Q_chunk_4d->view_offs + (size_t) h0 * Q_chunk_4d->nb[2]);
                                    q_ht = ggml_cont(ctx0, q_ht);
                                    ggml_tensor * v_ht = ggml_view_4d(ctx0, v_mat, v_mat->ne[0], v_mat->ne[1], hlen, v_mat->ne[3], v_mat->nb[1], v_mat->nb[2], v_mat->nb[3], v_mat->view_offs + (size_t) h0 * v_mat->nb[2]);
                                    v_ht = ggml_cont(ctx0, v_ht);
                                    ggml_tensor * KQ_ht = ggml_mul_mat(ctx0, k_ht, q_ht);
                                    ggml_mul_mat_set_prec(KQ_ht, GGML_PREC_DEFAULT);
                                    KQ_ht = ggml_soft_max_ext(ctx0, KQ_ht, mask_sub, kq_scale, 0.0f);
                                    ggml_tensor * KQV_ht = ggml_mul_mat(ctx0, v_ht, KQ_ht);
                                    ggml_tensor * tmp_perm = ggml_permute(ctx0, KQV_ht, 0, 2, 1, 3);
                                    ggml_tensor * tile_emb = ggml_cont_3d(ctx0, tmp_perm, d_head * hlen, qlen, batch_size);
                                    heads_cat = (heads_cat == nullptr) ? tile_emb : ggml_concat(ctx0, heads_cat, tile_emb, /*dim=*/0);
                                }

                                ggml_tensor * heads_cat_4d = ggml_reshape_4d(ctx0, heads_cat, n_embd, qlen, 1, batch_size);
                                ggml_tensor * heads_perm = ggml_permute(ctx0, heads_cat_4d, 0, 2, 1, 3);
                                ggml_tensor * chunk_out = ggml_cont_3d(ctx0, heads_perm, n_embd, qlen, batch_size);
                                out_cat = (out_cat == nullptr) ? chunk_out : ggml_concat(ctx0, out_cat, chunk_out, /*dim=*/1);
                            }
                        }

                        // Apply projection once to concatenated sequence
                        cur = ggml_cont(ctx0, out_cat);
                        if (layer.o_w) cur = ggml_mul_mat(ctx0, layer.o_w, cur);
...
private:
    ...
    // FA variant: expects K/V already permuted to [d_head{_v}, n_kv, n_head, B] and cast to F16.
    ggml_tensor * build_attn_flash_kv_prepared(
            ggml_tensor * wo, ggml_tensor * wo_b,
            ggml_tensor * q_cur, ggml_tensor * k_pre, ggml_tensor * v_pre,
            ggml_tensor * kq_mask, float kq_scale, int il) const {
        ggml_build_forward_expand(gf, q_cur);
        ggml_build_forward_expand(gf, k_pre);
        ggml_build_forward_expand(gf, v_pre);

        ggml_tensor * q = ggml_permute(ctx0, q_cur, 0, 2, 1, 3);
        GGML_ASSERT(k_pre->ne[2] == q->ne[2] && v_pre->ne[2] == q->ne[2]);

        const int64_t n_kv = k_pre->ne[1];
        if ((n_kv % 256) != 0) {
            LOG_ERR("%s: FA requires KV length multiple of 256, got %lld. Choose -cis so (cis/14)^2 %% 256==0.\n", __func__, (long long) n_kv);
            GGML_ASSERT(false && "FA requires KV length multiple of 256");
        }

        if (kq_mask) {
            const int64_t need_pad = GGML_PAD(q->ne[1], GGML_KQ_MASK_PAD);
            if (kq_mask->ne[1] < need_pad) {
                kq_mask = ggml_pad(ctx0, kq_mask, 0, (int)(need_pad - kq_mask->ne[1]), 0, 0);
            }
            if (kq_mask->type != GGML_TYPE_F16) kq_mask = ggml_cast(ctx0, kq_mask, GGML_TYPE_F16);
        }

        ggml_tensor * cur = ggml_flash_attn_ext(ctx0, q, k_pre, v_pre, kq_mask, kq_scale, 0.0f, 0.0f);
        if (!ggml_backend_supports_op(ctx->backend, cur)) {
            GGML_ASSERT(false && "FA not supported by backend");
        }
        ggml_flash_attn_ext_set_prec(cur, GGML_PREC_F32);
        cur = ggml_cont_2d(ctx0, cur, cur->ne[0]*cur->ne[1], cur->ne[2]);
        cb(cur, "kqv_out", il);
        if (wo) cur = ggml_mul_mat(ctx0, wo, cur);
        if (wo_b) cur = ggml_add(ctx0, cur, wo_b);
        return cur;
    }

    // FA variant: expects Q also pre-permuted to [d_head, n_tokens, n_head, B]
    ggml_tensor * build_attn_flash_qkv_prepared(
            ggml_tensor * wo, ggml_tensor * wo_b,
            ggml_tensor * q_pre, ggml_tensor * k_pre, ggml_tensor * v_pre,
            ggml_tensor * kq_mask, float kq_scale, int il) const {
        ggml_build_forward_expand(gf, q_pre);
        ggml_build_forward_expand(gf, k_pre);
        ggml_build_forward_expand(gf, v_pre);

        GGML_ASSERT(k_pre->ne[2] == q_pre->ne[2] && v_pre->ne[2] == q_pre->ne[2]);

        const int64_t n_kv = k_pre->ne[1];
        if ((n_kv % 256) != 0) {
            LOG_ERR("%s: FA requires KV length multiple of 256, got %lld.\n", __func__, (long long) n_kv);
            GGML_ASSERT(false && "FA requires KV length multiple of 256");
        }

        if (kq_mask) {
            const int64_t need_pad = GGML_PAD(q_pre->ne[1], GGML_KQ_MASK_PAD);
            if (kq_mask->ne[1] < need_pad) {
                kq_mask = ggml_pad(ctx0, kq_mask, 0, (int)(need_pad - kq_mask->ne[1]), 0, 0);
            }
            if (kq_mask->type != GGML_TYPE_F16) kq_mask = ggml_cast(ctx0, kq_mask, GGML_TYPE_F16);
        ...
        ggml_tensor * cur = ggml_flash_attn_ext(ctx0, q_pre, k_pre, v_pre, kq_mask, kq_scale, 0.0f, 0.0f);
        if (!ggml_backend_supports_op(ctx->backend, cur)) {
            GGML_ASSERT(false && "FA not supported by backend");
        }
        ggml_flash_attn_ext_set_prec(cur, GGML_PREC_F32);
    ...
    // Compute how many heads to process per tile step given a VRAM budget.
    static inline int64_t clip_calc_heads_per_step(size_t budget_bytes, int64_t n_kv, int64_t qlen,
                                                   int64_t n_head, size_t bytes_per, double overhead_ratio) {
        const size_t default_overhead = static_cast<size_t>(3543348019.0 * overhead_ratio);
        if (default_overhead >= budget_bytes) return 1;
        budget_bytes -= default_overhead;
        if (budget_bytes == 0 || n_kv <= 0 || qlen <= 0 || n_head <= 0) return 1;

        const size_t denom = bytes_per * (size_t) n_kv * (size_t) qlen;
        size_t h = denom ? (budget_bytes / denom) : 1;
        if (h < 1) h = 1;
        if ((int64_t) h > n_head) h = (size_t) n_head;

        // Prefer even divisor of n_head for balanced kernels
        size_t hd = h;
        while (hd > 1 && (n_head % (int64_t) hd) != 0) --hd;
        if (hd >= 1) h = hd;
        return (int64_t) h;
    }
...
struct clip_model_loader {
    ...
    void load_hparams(clip_model & model, clip_modality modality, int32_t clip_image_size = -1) {
        ...
        // other hparams
        {
            ...
            switch (model.proj_type) {
                ...
                case PROJECTOR_TYPE_QWEN2VL:
                    {
                        hparams.image_size = 1024;
                        if (clip_image_size != -1) {
                            hparams.image_size = clip_image_size;
                        }
                        if (mtmd_clip_flash_attn_enabled()) {
                            hparams.warmup_image_size = hparams.patch_size * 16;
                        } else {
                            hparams.warmup_image_size = hparams.patch_size * 8;
                        }
                    } break;
    ...
    void load_tensors(clip_ctx & ctx_clip) {
        ...
        // load data
        {
            ...
            if (ctx_clip.vto_offload_cpu)
            {
                // Offload all vision/clip tensors to CPU
                std::map<ggml_backend_buffer_type_t, std::vector<ggml_tensor *>> tensors_by_buft;
                LOG_INF("%s: VTO offload: placing all vision tensors on CPU\n", __func__);
                ...
                for (auto & [bt, tensor_list] : tensors_by_buft)
                {
                    struct ggml_init_params temp_params = {
                        tensor_list.size() * ggml_tensor_overhead(), NULL, true,
                    };
                    ggml_context_ptr temp_ctx(ggml_init(temp_params));
                    if (!temp_ctx) throw std::runtime_error(string_format("%s: failed to create temp context\n", __func__));

                    for (auto * orig : tensor_list) {
                        ggml_tensor * tmp = ggml_dup_tensor(temp_ctx.get(), orig);
                        ggml_set_name(tmp, orig->name);
                    }

                    auto buffer = ggml_backend_buffer_ptr(ggml_backend_alloc_ctx_tensors_from_buft(temp_ctx.get(), bt));
                    if (!buffer) throw std::runtime_error(string_format("%s: failed to alloc buffer for %s\n", __func__, ggml_backend_buft_name(bt)));
                    ggml_backend_buffer_set_usage(buffer.get(), GGML_BACKEND_BUFFER_USAGE_WEIGHTS);

                    for (auto * orig : tensor_list) {
                        ggml_tensor * tmp = ggml_get_tensor(temp_ctx.get(), orig->name);
                        orig->buffer = tmp->buffer;
                        orig->data   = tmp->data;
                    }
...
struct clip_init_result clip_init(const char * fname, struct clip_context_params ctx_params) {
    ...
    try {
        ...
        if (loader.has_vision) {
            ...
            loader.load_hparams(ctx_vision->model, CLIP_MODALITY_VISION, ctx_params.clip_img_size);
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs_c_ptr, float * vec) {
    ...
    // VTO: use a temporary scheduler so its compute buffers are freed after encode
    ...
    ggml_backend_sched_alloc_graph(sched_to_use, gf, alloc_memory);
    ...
    // VTO: free temporary scheduler to release compute buffers/VRAM
    if (ctx->vto_offload_cpu || ctx->vto_tiled_attention) {
