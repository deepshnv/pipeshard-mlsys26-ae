...
struct llama_model::impl {
    ...
    mutable layer_dev dev_temp = {};
    mutable std::vector<layer_dev> dev_layer;
	mutable std::vector<layer_dev> dev_layer_prompt_phase;											   
...
llama_model::~llama_model() {
    for (size_t i = 0; i < use_separate_cuda_host_buf.size(); ++i) {
        if (use_separate_cuda_host_buf[i] == false) {
            ggml_backend_reg_t reg = ggml_backend_dev_backend_reg(devices[0]);
            auto ggml_backend_unregister_host_buffer_fn = (ggml_backend_unregister_host_buffer)ggml_backend_reg_get_proc_address(reg, "ggml_backend_unregister_host_buffer");
            ggml_backend_unregister_host_buffer_fn(pimpl->mappings.at(i)->addr());
        }
    }    
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    const auto & use_mlock    = params.use_mlock;
          auto & tensor_split = params.tensor_split;
	      bool   pipe_shard   = params.pipe_shard;											  
	...
	if(pipe_shard){
		float tensor_split_[GGML_CUDA_MAX_SPLITS] = {0};
		for(int ts = 0; ts < n_devices(); ts++){
            if (ts < n_layer + 1) {
				tensor_split_[ts] = 1.0;
			}else{
				tensor_split_[ts] = 0.0;
			}
		}
		tensor_split = tensor_split_;
    }
    ...
    auto get_layer_buft_list = [&](int il) -> llama_model::impl::layer_dev {
        ...
        const int layer_gpu = std::upper_bound(splits.begin(), splits.begin() + device_count, float(il - i_gpu_start)/act_gpu_layers) - splits.begin();
	...
	pimpl->dev_layer_prompt_phase.resize(n_layer);
    for (int il = 0; il < n_layer; ++il) {
        pimpl->dev_layer[il] = get_layer_buft_list(il);
		pimpl->dev_layer_prompt_phase[il] = pimpl->dev_layer[il];
        if(il == 0) {pimpl->dev_temp = pimpl->dev_layer[il];}
    }
    ...
    // === HYBRID ALLOCATION: Classify tensors ===
    size_t total_pin_size = 0;
    ...
    if (pipe_shard) {
        struct TensorClassification {
            ...
            size_t size;
            int priority;        // Higher = more important to pin
            bool should_pin;     // Final decision
        };
    
        std::vector<TensorClassification> all_tensors;
    
        // Scan all contexts and classify tensors
        for (auto& it : ctx_map) {
            ...
            for (ggml_tensor* t = ggml_get_first_tensor(ctx); t; t = ggml_get_next_tensor(ctx, t)) {
                ...
                TensorClassification tc;
                tc.tensor = t;
                tc.ctx = ctx;
                tc.weight = weight;
                tc.size = ggml_nbytes(t);
                tc.should_pin = false;
            
                // Assign priority
                if (strstr(name, ".attn_")) {
                    tc.priority = 100;  // Critical
                } else if (strstr(name, ".ffn_")) {
                    tc.priority = 90;   // Low (leave mmaped)
                } else {
                    tc.priority = 100;   // Medium
                }
            
                all_tensors.push_back(tc);
            }
        }
        ...
        // Allocate pinned budget
        
        size_t max_pinned;
        if (params.pinned_sysmem_alloc == 0) {
            // Auto-calculate based on available system RAM using existing backend API
            ggml_backend_dev_t cpu_dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
            if (cpu_dev == nullptr) {
                throw std::runtime_error(format("%s: no CPU backend found", __func__));
            }
            ...
            // Use available RAM / 2.5 as budget (40% of available RAM)
            max_pinned = total_ram > 0 ? (size_t)(total_ram / 2.5) : (32ULL * 1024 * 1024 * 1024);
            
            LLAMA_LOG_INFO("%s: Auto-calculated pinned budget: %.2f GB (free now %.2f GB available RAM / %.2f GB total)\n",
                           __func__, max_pinned / (1024.0*1024.0*1024.0), free_ram / (1024.0*1024.0*1024.0), total_ram / (1024.0*1024.0*1024.0));
        } else {
            max_pinned = params.pinned_sysmem_alloc * 1024ULL * 1024ULL * 1024ULL;
            
            LLAMA_LOG_INFO("%s: User-specified pinned budget: %.2f GB\n",
                           __func__, max_pinned / (1024.0*1024.0*1024.0));
        }
        size_t total_pin_size = 0;
    
        for (auto& tc : all_tensors) {
            if (tc.priority >= 80 && total_pin_size + tc.size <= max_pinned) {
                tensor_pin_decisions[tc.tensor] = true;
                total_pin_size += tc.size;
            }else{
                tensor_pin_decisions[tc.tensor] = false;
                total_mmap_size += tc.size;
            }
        }
    
        LLAMA_LOG_INFO("%s: Hybrid strategy: %.2f GB pinned, %.2f GB mmaped (%.1f%% pinned)\n",
                      __func__, total_pin_size / (1024.0*1024.0*1024.0), 
                      (total_mmap_size) / (1024.0*1024.0*1024.0),
                       100.0 * total_pin_size / (total_pin_size + total_mmap_size));
    ...
    // === HYBRID ALLOCATION PATH ===
    if (pipe_shard && !tensor_pin_decisions.empty()) {
        // Collect ALL tensors from ALL contexts and separate by allocation strategy
        std::vector<ggml_tensor*> all_tensors_to_pin;
        std::vector<ggml_tensor*> all_tensors_to_mmap;
        std::map<ggml_tensor*, ggml_context*> tensor_to_ctx;  // Track which context owns each tensor
        ...
        for (auto & it : ctx_map) {
            ...
            for (ggml_tensor* t = ggml_get_first_tensor(ctx); t; t = ggml_get_next_tensor(ctx, t)) {
                auto decision_it = tensor_pin_decisions.find(t);
                if (decision_it != tensor_pin_decisions.end() && decision_it->second) {
                    all_tensors_to_pin.push_back(t);
                    tensor_to_ctx[t] = ctx;
                } else if (decision_it != tensor_pin_decisions.end()) {
                    all_tensors_to_mmap.push_back(t);
                    tensor_to_ctx[t] = ctx;
                }
            }
        }
        
        // Create ONE pinned buffer for ALL pinned tensors
        ggml_backend_buffer_t pinned_buf = nullptr;
        if (!all_tensors_to_pin.empty()) {
            auto * dev = devices.at(0);
            auto * host_buft = ggml_backend_dev_host_buffer_type(dev);
            
            // Calculate total size
            size_t pinned_buffer_size = 0;
            for (auto* t : all_tensors_to_pin) {
                pinned_buffer_size += ggml_nbytes(t);
            }
            
            pinned_buf = ggml_backend_buft_alloc_buffer(host_buft, pinned_buffer_size);
            
            if (!pinned_buf) {
                LLAMA_LOG_WARN("Failed to allocate %.2f GB pinned buffer, falling back to mmap\n",
                              pinned_buffer_size / (1024.0*1024.0*1024.0));
                // Fallback: treat all as mmap
                all_tensors_to_mmap.insert(all_tensors_to_mmap.end(), 
                                          all_tensors_to_pin.begin(), 
                                          all_tensors_to_pin.end());
                all_tensors_to_pin.clear();
            } else {
                // Copy from mmap to pinned
                size_t offset = 0;
                for (auto* t : all_tensors_to_pin) {
                    auto* weight = ml.get_weight(ggml_get_name(t));
                    auto& mapping = ml.mappings.at(weight->idx);
                    
                    void* mmap_data = (char*)mapping->addr() + weight->offs;
                    void* pinned_data = (char*)ggml_backend_buffer_get_base(pinned_buf) + offset;
                    
                    // Copy mmap → pinned
                    memcpy(pinned_data, mmap_data, ggml_nbytes(t));
                    
                    // Assign tensor to pinned buffer
                    ggml_backend_tensor_alloc(pinned_buf, t, pinned_data);
                    
                    hybrid_tensor_data[t] = pinned_data;

                    offset += ggml_nbytes(t);
                }
                
                pimpl->bufs.emplace_back(pinned_buf);
                ggml_backend_buffer_set_usage(pinned_buf, GGML_BACKEND_BUFFER_USAGE_WEIGHTS);
                
                LLAMA_LOG_INFO("%s: Created ONE pinned buffer: %.2f GB (%zu tensors)\n", __func__, pinned_buffer_size / (1024.0*1024.0*1024.0), all_tensors_to_pin.size());
            }
        }
        
        // Create mmap buffer(s) for remaining tensors (zero-copy)
        if (!all_tensors_to_mmap.empty()) {
            // Group by file index
            std::map<uint32_t, std::vector<ggml_tensor*>> tensors_by_file;
            for (auto* t : all_tensors_to_mmap) {
                auto* weight = ml.get_weight(ggml_get_name(t));
                tensors_by_file[weight->idx].push_back(t);
            }
            
            for (auto& file_tensors : tensors_by_file) {
                uint32_t idx = file_tensors.first;
                auto& tensors = file_tensors.second;
                ...
                for (auto* t : tensors) {
                    ...
                    last = std::max(last, weight->offs + ggml_nbytes(t));
                }
                ...
                ggml_backend_buffer_type_t compatible_buft = nullptr;
                bool buffer_from_host_ptr_supported = false;

                for (auto & it : ctx_map) {
                    ggml_backend_buffer_type_t buft = it.first;
                    ggml_backend_dev_t test_dev = ggml_backend_buft_get_device(buft);
    
                    if (!test_dev) {
                        test_dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
                    }
    
                    if (test_dev) {
                        ggml_backend_dev_props props;
                        ggml_backend_dev_get_props(test_dev, &props);
        
                        if (props.caps.buffer_from_host_ptr) {
                            dev = test_dev;
                            compatible_buft = buft;
                            buffer_from_host_ptr_supported = true;
                            break;  // Found one, use it
                        }
                    }
                }

                // Fallback if none found (shouldn't happen in pipe_shard)
                if (!dev) {
                    dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
                    LLAMA_LOG_WARN("%s: No device with buffer_from_host_ptr support found, using CPU\n", __func__);
                }
                ...
                if (mmap_buf) {
                    // Allocate tensors using zero-copy
                    for (auto* t : tensors) {
                        auto* weight = ml.get_weight(ggml_get_name(t));
                        void* tensor_data = (char*)addr + weight->offs;
                        ggml_backend_tensor_alloc(mmap_buf, t, tensor_data);

                        hybrid_tensor_data[t] = tensor_data;
                    }
                    
                    pimpl->bufs.emplace_back(mmap_buf);
                    ggml_backend_buffer_set_usage(mmap_buf, GGML_BACKEND_BUFFER_USAGE_WEIGHTS);
                    
                    LLAMA_LOG_INFO("%s: Created mmap buffer for file %u: %.2f GB (%zu tensors)\n",
                                  __func__, idx, (last - first) / (1024.0*1024.0*1024.0), tensors.size());
                }
            }
        }
        
        // Create ctx_bufs entries for each context
        for (auto & it : ctx_map) {
            ggml_context * ctx = it.second;
            if (ggml_get_first_tensor(ctx) == nullptr) {
                continue;
            ...
            // Map all tensors in this context to their respective buffers
            if (pinned_buf) {
                buf_map.emplace(0, pinned_buf);
            }
            ctx_bufs.emplace_back(ctx, buf_map);
        }
    } else {
        // === NORMAL ALLOCATION PATH (non-hybrid) ===
        for (auto & it : ctx_map) {
            ggml_backend_buffer_type_t buft = it.first;
            ggml_context * ctx              = it.second;
            ...
            if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
                for (uint32_t idx = 0; idx < ml.files.size(); idx++) {
                    ...
                    if (pipe_shard) {
                        ...
                        buf = ggml_backend_dev_buffer_from_host_ptr(dev, (char *) ml.mappings.at(idx)->addr(), ml.mappings.at(idx)->size(), ml.mappings.at(idx)->size());
                    } else {
                        void * addr = nullptr;
                        size_t first, last;
                ...
                ggml_backend_buffer_t buf = nullptr;
                if (pipe_shard) {
                    if (ml.use_mmap == false) {
                        auto * dev       = devices.at(0);
                        auto * host_buft = ggml_backend_dev_host_buffer_type(dev);
                        buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx, host_buft);
                    } else {
            ...
            if (pipe_shard == true && ml.use_mmap == true) {
                if (buffer_from_host_ptr_supported) {
                    ctx_bufs.emplace_back(ctx, buf_map);
                }
            } else {
                ctx_bufs.emplace_back(ctx, buf_map);
    ...
    for (auto & buf : pimpl->bufs) {
        if (ggml_backend_buffer_get_size(buf.get()) != 0) {
            std::string backend = (pipe_shard == true && ml.use_mmap == false) ? "CUDA_Host_Pinned_Mem" : ggml_backend_buffer_name(buf.get());
            LLAMA_LOG_INFO("%s: %12s model buffer size = %8.2f MiB\n", __func__, backend.c_str(), ggml_backend_buffer_get_size(buf.get()) / 1024.0 / 1024.0);
        }
    }

    // load tensor data
    // Skip for hybrid mode - we already loaded pinned (via memcpy) and mmap (via zero-copy)
    if (!pipe_shard || tensor_pin_decisions.empty()) {
        ...
        LLAMA_LOG_INFO("%s: Skipping ml.load_all_data for hybrid mode (already loaded manually)\n", __func__);
    }

    std::vector<size_t> mapping_size;
    int num_maps = 0;
    if (use_mmap_buffer) {
        for (auto & mapping : ml.mappings) {
            mapping_size.push_back(mapping->size());
    ...
    for (auto & ctx : pimpl->ctxs) {
        for (auto * cur = ggml_get_first_tensor(ctx.get()); cur != NULL; cur = ggml_get_next_tensor(ctx.get(), cur)) {
            ...
            // Parse the layer index from the tensor name and populate the map
            int32_t layer_idx = GGML_CUDA_MAX_DEVICES;  // Default for non-layer tensors

            std::string tensor_name = ggml_get_name(cur);
            size_t pos = tensor_name.find("blk.");
            if (pos != std::string::npos) {
                pos += 4;  // Move past "blk."
                size_t end_pos = tensor_name.find('.', pos);
                if (end_pos != std::string::npos) {
                    layer_idx = std::stoi(tensor_name.substr(pos, end_pos - pos));
                }
            }
...
ggml_backend_dev_t llama_model::dev_input() const {
...
void* llama_model::get_mapping_addr(int i) const {
    return pimpl->mappings.at(i)->addr();
}

ggml_backend_buffer_t llama_model::get_cpu_buf() const {
    return pimpl->bufs.back().get();
}
